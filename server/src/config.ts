...
const TASKCORE_ENV_FILE_PATH = resolveTaskcoreEnvPath();
if (existsSync(TASKCORE_ENV_FILE_PATH)) {
  loadDotenv({ path: TASKCORE_ENV_FILE_PATH, override: false, quiet: true });
}
...
function detectTailnetBindHost(): string | undefined {
  const explicit = process.env.TASKCORE_TAILNET_BIND_HOST?.trim();
...
export function loadConfig(): Config {
  ...
  const storageProviderFromEnvRaw = process.env.TASKCORE_STORAGE_PROVIDER;
  ...
  const storageS3Endpoint = process.env.TASKCORE_STORAGE_S3_ENDPOINT ?? fileStorage?.s3?.endpoint ?? undefined;
  const storageS3Prefix = process.env.TASKCORE_STORAGE_S3_PREFIX ?? fileStorage?.s3?.prefix ?? "";
  const storageS3ForcePathStyle =
    process.env.TASKCORE_STORAGE_S3_FORCE_PATH_STYLE !== undefined
      ? process.env.TASKCORE_STORAGE_S3_FORCE_PATH_STYLE === "true"
      : (fileStorage?.s3?.forcePathStyle ?? false);
  const feedbackExportBackendUrl =
    process.env.TASKCORE_FEEDBACK_EXPORT_BACKEND_URL?.trim() ||
    process.env.TASKCORE_TELEMETRY_BACKEND_URL?.trim() ||
    undefined;
  const feedbackExportBackendToken =
    process.env.TASKCORE_FEEDBACK_EXPORT_BACKEND_TOKEN?.trim() ||
  ...
  const bindFromEnvRaw = process.env.TASKCORE_BIND;
  const bindFromEnv =
  ...
  const customBindHost = process.env.TASKCORE_BIND_HOST ?? fileConfig?.server.customBindHost;
  ...
  const allowedHostnamesFromEnvRaw = process.env.TASKCORE_ALLOWED_HOSTNAMES;
  ...
  const companyDeletionEnvRaw = process.env.TASKCORE_ENABLE_COMPANY_DELETION;
  const companyDeletionEnabled =
    companyDeletionEnvRaw !== undefined
      ? companyDeletionEnvRaw === "true"
      : deploymentMode === "local_trusted";
  const databaseBackupEnabled =
    process.env.TASKCORE_DB_BACKUP_ENABLED !== undefined
      ? process.env.TASKCORE_DB_BACKUP_ENABLED === "true"
  ...
  return {
    ...
    databaseUrl: resolvePostgresUrlFromEnv() ?? fileDbUrl,
    ...
    uiDevMiddleware: process.env.TASKCORE_UI_DEV_MIDDLEWARE === "true",
    secretsProvider,
    secretsStrictMode,
