...
import type { TaskcoreConfig } from "@taskcore/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Taskcore worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.TASKCORE_IN_WORKTREE !== "true") return null;
...
function writeConfigFile(configPath: string, config: TaskcoreConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".taskcore", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as TaskcoreConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: TaskcoreConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: TaskcoreConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.TASKCORE_HOME = context.homeDir;
  process.env.TASKCORE_INSTANCE_ID = context.instanceId;
  process.env.TASKCORE_CONFIG = context.configPath;
  process.env.TASKCORE_CONTEXT = context.contextPath;
  process.env.TASKCORE_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as TaskcoreConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    TASKCORE_HOME: context.homeDir,
    TASKCORE_INSTANCE_ID: context.instanceId,
    TASKCORE_CONFIG: context.configPath,
    TASKCORE_CONTEXT: context.contextPath,
    TASKCORE_IN_WORKTREE: "true",
    TASKCORE_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: TaskcoreConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as TaskcoreConfig;
  } catch {
    return;
  }
