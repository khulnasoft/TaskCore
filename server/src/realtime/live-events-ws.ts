...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@taskcore/db";
import type { DeploymentMode } from "@taskcore/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  taskcoreUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).taskcoreUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.taskcoreUpgradeContext = context;
