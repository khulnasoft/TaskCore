...
describe("agent local JWT", () => {
  const secretEnv = "TASKCORE_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "TASKCORE_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "TASKCORE_AGENT_JWT_ISSUER";
  const audienceEnv = "TASKCORE_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when TASKCORE_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "taskcore";
    process.env[audienceEnv] = "taskcore-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
