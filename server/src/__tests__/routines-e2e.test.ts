...
describeEmbeddedPostgres("routine routes end-to-end", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("taskcore-routines-e2e-");
  ...
  beforeEach(() => {
    vi.resetModules();
    vi.doUnmock("@taskcore/shared/telemetry");
  ...
  it("runs routines with variable inputs and interpolates the execution issue description", async () => {
    ...
    expect(issue?.description).toBe("Review taskcore for high bugs");
  });
