...
import { testEnvironment } from "@taskcore/adapter-opencode-local/server";
...
describe("opencode_local environment diagnostics", () => {
  it("reports a missing working directory as an error when cwd is absolute", async () => {
    const cwd = path.join(
      os.tmpdir(),
      `taskcore-opencode-local-cwd-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      "workspace",
    );
  ...
  it("classifies ProviderModelNotFoundError probe output as model-unavailable warning", async () => {
    ...
    const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "taskcore-opencode-env-probe-bin-"));
