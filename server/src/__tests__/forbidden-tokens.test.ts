...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "taskcore", LOGNAME: "taskcore", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "taskcore" }),
      },
    );

    expect(tokens).toEqual(["taskcore", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "taskcore" }, {
        userInfo: () => ({ username: "taskcore" }),
      });

      expect(tokens).toEqual(["taskcore", "custom-token"]);
    } finally {
