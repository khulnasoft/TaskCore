...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Taskcore OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Taskcore base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("taskcoreApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.taskcoreApiUrl");
    expect(text).toContain("~/.openclaw/workspace/taskcore-claimed-api-key.json");
    expect(text).toContain("TASKCORE_API_KEY");
