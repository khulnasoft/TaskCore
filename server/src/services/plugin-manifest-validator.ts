 ...
 */
import { pluginManifestV1Schema } from "@taskcore/shared";
import type { TaskcorePluginManifestV1 } from "@taskcore/shared";
import { PLUGIN_API_VERSION } from "@taskcore/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): TaskcorePluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as TaskcorePluginManifestV1,
        };
      }
