 ...
 *    (`~/.taskcore/plugins/`) and `node_modules` for packages matching
 *    the `taskcore-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Taskcore plugins.
 * Packages matching this pattern are considered Taskcore plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "taskcore-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: TaskcorePluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.taskcore/plugins/ local directory
  | "npm"               // npm packages matching taskcore-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: TaskcorePluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the taskcore-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "taskcore-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: TaskcorePluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Taskcore plugins installed as npm packages in the current
   * Node.js environment matching the "taskcore-plugin-*" naming convention.
   *
   ...
   * the "taskcorePlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Taskcore plugin.
   * Throws if the package is a Taskcore plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<TaskcorePluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: TaskcorePluginManifestV1;
    newManifest: TaskcorePluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Taskcore plugin naming convention.
 * Accepts both the "taskcore-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @taskcore/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "taskcorePlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const taskcorePlugin = pkgJson["taskcorePlugin"];
  if (
    taskcorePlugin !== null &&
    typeof taskcorePlugin === "object" &&
    !Array.isArray(taskcorePlugin)
  ) {
    const manifestRelPath = (taskcorePlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: TaskcorePluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumTaskcoreVersion;
}
 ...
 *   packageName: "taskcore-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: TaskcorePluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as TaskcorePluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Taskcore plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<TaskcorePluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Taskcore plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasTaskcorePlugin = "taskcorePlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasTaskcorePlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<TaskcorePluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: TaskcorePluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @taskcore/shared exports). Run those workers through
