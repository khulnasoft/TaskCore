...
import { readTaskcoreSkillSyncPreference } from "@taskcore/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@taskcore/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@taskcore/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@taskcore/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveTaskcoreInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "taskcore-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "taskcore-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "taskcore-feedback-v1";
const FEEDBACK_DESTINATION = "taskcore_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "taskcore_run" &&
    file.source !== "taskcore_run_events" &&
    file.source !== "taskcore_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.TASKCORE_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readTaskcoreSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let taskcoreRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      taskcoreRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "taskcore/run.json",
        contentType: "application/json",
        source: "taskcore_run",
        contents: `${JSON.stringify(taskcoreRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.taskcoreRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "taskcore/run-events.json",
        contentType: "application/json",
        source: "taskcore_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "taskcore/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "taskcore_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.taskcoreRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
