...
import type { TaskcorePluginManifestV1 } from "@taskcore/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __taskcore_exports: module.exports,
      __taskcore_module: module,
      __taskcore_require: requireInSandbox,
      __taskcore_filename: realPath,
      __taskcore_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__taskcore_exports, __taskcore_module, __taskcore_require, __taskcore_filename, __taskcore_dirname)`;
