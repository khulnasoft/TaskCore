...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@taskcore/db";
 ...
 *
 * This implementation delegates to the core Taskcore domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@taskcore/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
