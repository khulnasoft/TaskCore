...
import { companySkills } from "@taskcore/db";
import { readTaskcoreSkillSyncPreference } from "@taskcore/adapter-utils/server-utils";
import type { TaskcoreSkillEntry } from "@taskcore/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@taskcore/shared";
import { findActiveServerAdapter } from "../adapters/index.js";
import { resolveTaskcoreInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("taskcore/taskcore/")) return slug;
...
function resolveDesiredSkillKeys(
  ...
  const preference = readTaskcoreSkillSyncPreference(config);
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveTaskcoreInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "taskcore_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Taskcore skills are read-only.",
      sourceLabel: "Taskcore bundled",
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "Taskcore workspace",
        sourceBadge: "taskcore",
...
export function companySkillService(db: Db) {
  ...
  ): Promise<TaskcoreSkillEntry[]> {
    const skills = await listFull(companyId);

    const out: TaskcoreSkillEntry[] = [];
    for (const skill of skills) {
      ...
      const required = sourceKind === "taskcore_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled Taskcore skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "taskcore_bundled"
        && incomingKind === "github"
        && incomingOwner === "taskcore"
        && incomingRepo === "taskcore"
      ) {
        out.push(existing);
        continue;
      }
