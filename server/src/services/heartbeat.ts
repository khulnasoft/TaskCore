...
import { trackAgentFirstHeartbeat } from "@taskcore/shared/telemetry";
...
import { extractSkillMentionIds } from "@taskcore/shared";

const MAX_LIVE_LOG_CHUNK_BYTES = 8 * 1024;
const MAX_PERSISTED_LOG_CHUNK_CHARS = 64 * 1024;
const HEARTBEAT_MAX_CONCURRENT_RUNS_DEFAULT = 1;
const HEARTBEAT_MAX_CONCURRENT_RUNS_MAX = 10;
const DEFERRED_WAKE_CONTEXT_KEY = "_taskcoreWakeContext";
const WAKE_COMMENT_IDS_KEY = "wakeCommentIds";
const TASKCORE_WAKE_PAYLOAD_KEY = "taskcoreWake";
const TASKCORE_HARNESS_CHECKOUT_KEY = "taskcoreHarnessCheckedOut";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__taskcore_repo_only__";
...
export function applyRunScopedMentionedSkillKeys(
  ...
  const existingPreference = readTaskcoreSkillSyncPreference(config);
  return writeTaskcoreSkillSyncPreference(config, [
...
export function compactRunLogChunk(chunk: string, maxChars = MAX_PERSISTED_LOG_CHUNK_CHARS) {
  ...
  const marker = `\n[taskcore truncated run log chunk: omitted ${omittedChars} chars]\n`;
...
function enrichWakeContextSnapshot(input: {
  ...
  if (wakeCommentIds.length > 0) {
    ...
    delete contextSnapshot[TASKCORE_WAKE_PAYLOAD_KEY];
...
export function mergeCoalescedContextSnapshot(
  ...
  if (mergedCommentIds.length > 0) {
    ...
    delete merged[TASKCORE_WAKE_PAYLOAD_KEY];
  }
  return merged;
}

async function buildTaskcoreWakePayload(input: {
  db: Db;
  companyId: string;
  contextSnapshot: Record<string, unknown>;
  issueSummary?:
  ...
  return {
    ...
    issue: issueSummary
      ...
      : null,
    checkedOutByHarness: input.contextSnapshot[TASKCORE_HARNESS_CHECKOUT_KEY] === true,
    ...
    for (const issue of candidates) {
      ...
      if (issue.status === "todo") {
        ...
        if (latestRetryReason === "assignment_recovery") {
          const updated = await escalateStrandedAssignedIssue({
            ...
            latestRun,
            comment:
              "Taskcore automatically retried dispatch for this assigned `todo` issue after a lost wake/run, " +
      ...
      if (latestRetryReason === "issue_continuation_needed") {
        const updated = await escalateStrandedAssignedIssue({
          ...
          latestRun,
          comment:
            "Taskcore automatically retried continuation for this assigned `in_progress` issue after its live " +
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          context[TASKCORE_HARNESS_CHECKOUT_KEY] = true;
        } catch (error) {
          if (!isCheckoutConflictError(error)) throw error;
          context[TASKCORE_HARNESS_CHECKOUT_KEY] = false;
        }
      ...
      const issueRef = issueContext
        ...
        : null;
      const taskcoreWakePayload = await buildTaskcoreWakePayload({
        db,
        companyId: agent.companyId,
        contextSnapshot: context,
      ...
      if (taskcoreWakePayload) {
        context[TASKCORE_WAKE_PAYLOAD_KEY] = taskcoreWakePayload;
      } else {
        delete context[TASKCORE_WAKE_PAYLOAD_KEY];
      }
      ...
      const runtimeConfig = {
        ...
        taskcoreRuntimeSkills: runtimeSkillEntries,
      };
      ...
      context.taskcoreWorkspaces = resolvedWorkspace.workspaceHints;
      ...
      if (runtimeServiceIntents.length > 0) {
        context.taskcoreRuntimeServiceIntents = runtimeServiceIntents;
      } else {
        delete context.taskcoreRuntimeServiceIntents;
      }
      ...
      if (sessionCompaction.rotate) {
        context.taskcoreSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
        context.taskcoreSessionRotationReason = sessionCompaction.reason;
        context.taskcorePreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
        ...
        delete context.taskcoreSessionHandoffMarkdown;
        delete context.taskcoreSessionRotationReason;
      ...
      try {
        ...
        if (runScopedMentionedSkillKeys.length > 0) {
          await onLog(
            "stdout",
            `[taskcore] Enabled run-scoped skills from issue mentions: ${runScopedMentionedSkillKeys.join(", ")}\n`,
          );
        }
        ...
        if (runtimeServices.length > 0) {
          context.taskcoreRuntimeServices = runtimeServices;
          context.taskcoreRuntimePrimaryUrl =
        ...
        if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
          try {
            ...
            await onLog(
              "stderr",
              `[taskcore] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
        ...
        if (adapter.supportsLocalAgentJwt && !authToken) {
          logger.warn(
            ...
            "local agent jwt secret missing or invalid; running without injected TASKCORE_API_KEY",
          );
        }
        ...
        if (adapterManagedRuntimeServices.length > 0) {
          ...
          context.taskcoreRuntimeServices = combinedRuntimeServices;
          context.taskcoreRuntimePrimaryUrl =
          ...
          if (issueId) {
            try {
              ...
              await onLog(
                "stderr",
                `[taskcore] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
              );
            }
          }
        }
        ...
        if (finalizedRun) {
          ...
          if (issueId && outcome === "succeeded") {
            try {
              ...
              await onLog(
                "stderr",
                `[taskcore] Failed to post run summary comment: ${err instanceof Error ? err.message : String(err)}\n`,
              );
            }
          }
