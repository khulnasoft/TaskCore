...
import type { AdapterRuntimeServiceReport } from "@taskcore/adapter-utils";
import type { Db } from "@taskcore/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@taskcore/db";
import {
...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".taskcore", "dist", "node_modules"]);
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.TASKCORE_WORKSPACE_CWD = input.worktreePath;
  env.TASKCORE_WORKSPACE_PATH = input.worktreePath;
  env.TASKCORE_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.TASKCORE_WORKSPACE_BRANCH = input.branchName;
  env.TASKCORE_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.TASKCORE_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.TASKCORE_WORKSPACE_SOURCE = input.base.source;
  env.TASKCORE_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.TASKCORE_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.TASKCORE_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.TASKCORE_PROJECT_ID = input.base.projectId ?? "";
  env.TASKCORE_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.TASKCORE_AGENT_ID = input.agent.id ?? "";
  env.TASKCORE_AGENT_NAME = input.agent.name;
  env.TASKCORE_COMPANY_ID = input.agent.companyId;
  env.TASKCORE_ISSUE_ID = input.issue?.id ?? "";
  env.TASKCORE_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.TASKCORE_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.TASKCORE_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.TASKCORE_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.TASKCORE_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.TASKCORE_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.TASKCORE_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.TASKCORE_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.TASKCORE_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.TASKCORE_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.TASKCORE_PROJECT_ID = input.workspace.projectId ?? "";
  env.TASKCORE_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.TASKCORE_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".taskcore", "worktrees");
