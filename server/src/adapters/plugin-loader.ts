...
function extractUiParserSource(
  ...
  const contractVersion = pkg.taskcore?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no taskcore.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
