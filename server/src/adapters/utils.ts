...
import * as serverUtils from "@taskcore/adapter-utils/server-utils";
export type { RunProcessResult } from "@taskcore/adapter-utils/server-utils";
...
export const buildTaskcoreEnv = serverUtils.buildTaskcoreEnv;
...
export function buildInvocationEnvForLogs(
  ...
  // TODO: Remove this fallback once @taskcore/adapter-utils exports buildInvocationEnvForLogs everywhere we consume it.
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "TASKCORE_RESOLVED_COMMAND"] = resolvedCommand;
  }
