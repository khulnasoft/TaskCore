...
import { getAdapterSessionManagement } from "@taskcore/adapter-utils";
import {
...
} from "@taskcore/adapter-claude-local/server";
import { agentConfigurationDoc as claudeAgentConfigurationDoc, models as claudeModels } from "@taskcore/adapter-claude-local";
import {
...
import { agentConfigurationDoc as codexAgentConfigurationDoc, models as codexModels } from "@taskcore/adapter-codex-local";
import {
...
import { agentConfigurationDoc as cursorAgentConfigurationDoc, models as cursorModels } from "@taskcore/adapter-cursor-local";
import {
...
import { agentConfigurationDoc as geminiAgentConfigurationDoc, models as geminiModels } from "@taskcore/adapter-gemini-local";
import {
  ...
  listOpenCodeModels,
} from "@taskcore/adapter-opencode-local/server";
import {
  agentConfigurationDoc as openCodeAgentConfigurationDoc,
  models as openCodeModels,
} from "@taskcore/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@taskcore/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@taskcore/adapter-openclaw-gateway";
...
} from "@taskcore/adapter-pi-local";
import {
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[taskcore] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[taskcore] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[taskcore] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[taskcore] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
