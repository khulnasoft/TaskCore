...
import { trackProjectCreated } from "@taskcore/shared/telemetry";
...
export function projectRoutes(db: Db) {
  ...
  async function handleProjectWorkspaceRuntimeCommand(req: Request, res: Response) {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Project workspace needs a local path before Taskcore can run workspace commands" });
      return;
    }
