...
import type { DeploymentExposure, DeploymentMode } from "@taskcore/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function resolveTaskcoreSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const taskcoreSkillNames = new Set<string>();
  if (taskcoreSkillsDir) {
    try {
      for (const entry of fs.readdirSync(taskcoreSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) taskcoreSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.taskcoreApiUrl)) {
    const legacyTaskcoreApiUrl = nonEmptyTrimmedString(input.taskcoreApiUrl);
    if (legacyTaskcoreApiUrl) merged.taskcoreApiUrl = legacyTaskcoreApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    taskcoreApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.taskcoreApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawTaskcoreApiUrl =
    typeof defaults.taskcoreApiUrl === "string"
      ? defaults.taskcoreApiUrl.trim()
      : "";
  if (rawTaskcoreApiUrl) {
    try {
      const parsedTaskcoreApiUrl = new URL(rawTaskcoreApiUrl);
      if (
        parsedTaskcoreApiUrl.protocol !== "http:" &&
        parsedTaskcoreApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_taskcore_api_url_protocol",
          level: "warn",
          message: `taskcoreApiUrl must use http:// or https:// (got ${parsedTaskcoreApiUrl.protocol}).`
        });
      } else {
        normalized.taskcoreApiUrl = parsedTaskcoreApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_taskcore_api_url_configured",
          level: "info",
          message: `taskcoreApiUrl set to ${parsedTaskcoreApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_taskcore_api_url_invalid",
        level: "warn",
        message: `Invalid taskcoreApiUrl: ${rawTaskcoreApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Taskcore host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Taskcore is bound to loopback in authenticated/private mode.",
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/taskcore";
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    The board approves the join request in Taskcore before key claim is allowed.
    ...
    "Ensure Taskcore is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Taskcore base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.taskcoreApiUrl when submitting your join request
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@taskcore.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "taskcore", path: "/api/skills/taskcore" },
        {
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.taskcoreApiUrl &&
          !persistedDefaults.taskcoreApiUrl
        ) {
          missingPersistedFields.push("taskcoreApiUrl");
        }
