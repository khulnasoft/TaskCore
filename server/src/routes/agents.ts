...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable } from "@taskcore/db";
...
import { trackAgentCreated } from "@taskcore/shared/telemetry";
...
import { runClaudeLogin } from "@taskcore/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@taskcore/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@taskcore/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@taskcore/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@taskcore/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.TASKCORE_SECRETS_STRICT_MODE === "true";
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeTaskcoreSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readTaskcoreSkillSyncPreference(
