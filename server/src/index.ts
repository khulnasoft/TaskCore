...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.TASKCORE_SECRETS_PROVIDER === undefined) {
    process.env.TASKCORE_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.TASKCORE_SECRETS_STRICT_MODE === undefined) {
    process.env.TASKCORE_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.TASKCORE_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.TASKCORE_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.TASKCORE_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.TASKCORE_MIGRATION_PROMPT === "never") return false;
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@taskcore.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.TASKCORE_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://taskcore:taskcore@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "taskcore");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://taskcore:taskcore@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "taskcore");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: taskcore");
    }

    const embeddedConnectionString = `postgres://taskcore:taskcore@127.0.0.1:${port}/taskcore`;
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.TASKCORE_LISTEN_HOST = runtimeListenHost;
  process.env.TASKCORE_LISTEN_PORT = String(listenPort);
  process.env.TASKCORE_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.TASKCORE_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.TASKCORE_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Taskcore server failed to start");
    process.exit(1);
  });
}
