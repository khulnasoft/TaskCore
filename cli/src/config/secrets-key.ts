...
import type { TaskcoreConfig } from "./schema.js";
...
export function ensureLocalSecretsKeyFile(
  config: Pick<TaskcoreConfig, "secrets">,
  configPath?: string,
  ...
  const envMasterKey = process.env.TASKCORE_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    return { status: "skipped_env", path: null };
  }

  const keyFileOverride = process.env.TASKCORE_SECRETS_MASTER_KEY_FILE;
