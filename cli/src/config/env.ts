...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Taskcore CLI commands",
...
export function resolveTaskcoreEnvFile(configPath?: string): string {
...
export function loadTaskcoreEnvFile(configPath?: string): void {
...
export function mergeTaskcoreEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readTaskcoreEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeTaskcoreEnvEntries(next, filePath);
  return next;
}
