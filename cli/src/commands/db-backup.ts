...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@taskcore/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): {
  ...
  return {
    value: `postgres://taskcore:taskcore@127.0.0.1:${port}/taskcore`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" taskcore db:backup ")));
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "taskcore";
