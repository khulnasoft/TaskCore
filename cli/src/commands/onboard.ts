...
const ONBOARD_ENV_KEYS = [
  "TASKCORE_PUBLIC_URL",
  "DATABASE_URL",
  "TASKCORE_DB_BACKUP_ENABLED",
  "TASKCORE_DB_BACKUP_INTERVAL_MINUTES",
  "TASKCORE_DB_BACKUP_RETENTION_DAYS",
  "TASKCORE_DB_BACKUP_DIR",
  "TASKCORE_DEPLOYMENT_MODE",
  "TASKCORE_DEPLOYMENT_EXPOSURE",
  "TASKCORE_BIND",
  "TASKCORE_BIND_HOST",
  "TASKCORE_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "TASKCORE_ALLOWED_HOSTNAMES",
  "TASKCORE_AUTH_BASE_URL_MODE",
  "TASKCORE_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "TASKCORE_STORAGE_PROVIDER",
  "TASKCORE_STORAGE_LOCAL_DIR",
  ...
  "TASKCORE_SECRETS_PROVIDER",
  "TASKCORE_SECRETS_STRICT_MODE",
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.TASKCORE_TAILNET_BIND_HOST?.trim(),
  });
  ...
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.TASKCORE_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" taskcore onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveTaskcoreInstanceId());
  p.log.message(
    pc.dim(
  ...
  if (existingConfig) {
    ...
    p.note(
      [
        ...
        "Agent auth: TASKCORE_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("taskcore run")}`,
        `Reconfigure later: ${pc.cyan("taskcore configure")}`,
        `Diagnose setup: ${pc.cyan("taskcore doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    p.outro("Existing Taskcore setup is ready.");
    return;
  }
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@taskcore/db");
  ...
  if (
    ...
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("taskcore run")}`,
        `Then: ${pc.cyan("taskcore auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
