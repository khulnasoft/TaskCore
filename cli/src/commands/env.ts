...
const DEFAULT_AGENT_JWT_ISSUER = "taskcore";
const DEFAULT_AGENT_JWT_AUDIENCE = "taskcore-api";
...
export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" taskcore env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: TaskcoreConfig | null = null;
...
function collectDeploymentEnvRows(
  ...
  const secretsStrictMode =
    process.env.TASKCORE_SECRETS_STRICT_MODE ??
  ...
  const storageS3Bucket =
    process.env.TASKCORE_STORAGE_S3_BUCKET ??
    config?.storage?.s3?.bucket ??
    "taskcore";
  const storageS3Region =
    process.env.TASKCORE_STORAGE_S3_REGION ??
  ...
  const rows: EnvVarRow[] = [
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from TASKCORE_PUBLIC_URL when possible)",
    },
    {
    ...
    {
      key: "TASKCORE_AGENT_JWT_ISSUER",
      value: process.env.TASKCORE_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.TASKCORE_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
    ...
    {
      key: "TASKCORE_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.TASKCORE_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "TASKCORE_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.TASKCORE_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "TASKCORE_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.TASKCORE_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "TASKCORE_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.TASKCORE_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "TASKCORE_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.TASKCORE_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "TASKCORE_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.TASKCORE_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "TASKCORE_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.TASKCORE_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "TASKCORE_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.TASKCORE_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "TASKCORE_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.TASKCORE_STORAGE_S3_PREFIX
        ? "env"
    ...
    {
      key: "TASKCORE_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.TASKCORE_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.TASKCORE_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "TASKCORE_CONFIG",
      value: process.env.TASKCORE_CONFIG ?? configPath,
      source: process.env.TASKCORE_CONFIG ? "env" : "default",
      required: false,
