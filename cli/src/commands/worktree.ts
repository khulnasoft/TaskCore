...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.TASKCORE_CONFIG;
...
const WORKTREE_NAME_PREFIX = "taskcore-";
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromTaskcoreConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.taskcore${path.sep}worktrees${path.sep}`;
...
export function resolveWorktreeReseedTargetPaths(input: {
  ...
  if (!homeDir || !instanceId) {
    throw new Error(
      `Target config ${input.configPath} does not look like a worktree-local Taskcore instance. Expected TASKCORE_HOME and TASKCORE_INSTANCE_ID in the adjacent .env.`,
    );
  }
...
async function ensureRepairTargetWorktree(input: {
  ...
  return {
    rootPath: targetPath,
    configPath: path.resolve(targetPath, ".taskcore", "config.json"),
    label: branchName,
    branchName,
    created: true,
  };
...
function resolveSourceConnectionString(
  ...
  return `postgres://taskcore:taskcore@127.0.0.1:${port}/taskcore`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: TaskcoreConfig;
...
async function seedWorktreeDatabase(input: {
  ...
  const sourceEnvEntries = readTaskcoreEnvEntries(sourceEnvFile);
  ...
  try {
    if (input.sourceConfig.database.mode === "embedded-postgres") {
      ...
      const sourceAdminConnectionString = `postgres://taskcore:taskcore@127.0.0.1:${sourceHandle.port}/postgres`;
      await ensurePostgresDatabase(sourceAdminConnectionString, "taskcore");
    }
    ...
    const adminConnectionString = `postgres://taskcore:taskcore@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "taskcore");
    const targetConnectionString = `postgres://taskcore:taskcore@127.0.0.1:${targetHandle.port}/taskcore`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  loadTaskcoreEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Taskcore inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Taskcore once) before using worktree merge history.`,
      );
    }
...
function resolveEndpointFromChoice(
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".taskcore", "config.json"),
...
async function runWorktreeReseed(opts: WorktreeReseedOptions): Promise<void> {
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop Taskcore in ${targetEndpoint.rootPath} before reseeding, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
