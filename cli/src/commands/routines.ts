...
async function openConfiguredDb(configPath: string): Promise<{
  ...
  try {
    if (config.database.mode === "embedded-postgres") {
      ...
      const adminConnectionString = `postgres://taskcore:taskcore@127.0.0.1:${embeddedHandle.port}/postgres`;
      await ensurePostgresDatabase(adminConnectionString, "taskcore");
      const connectionString = `postgres://taskcore:taskcore@127.0.0.1:${embeddedHandle.port}/taskcore`;
