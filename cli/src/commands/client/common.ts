...
export function resolveCommandContext(
  ...
  const companyId =
    ...
    process.env.TASKCORE_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set TASKCORE_COMPANY_ID, or set context profile companyId via `taskcore context set`.",
    );
  }
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.TASKCORE_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.TASKCORE_SERVER_PORT || "");
