...
import type { Agent } from "@taskcore/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveTaskcoreSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    ...
    `export TASKCORE_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Taskcore skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Taskcore skills into ~/.codex/skills and ~/.claude/skills",
      )
