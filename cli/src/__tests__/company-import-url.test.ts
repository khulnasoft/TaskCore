...
describe("isGithubShorthand", () => {
  it("matches owner/repo/path shorthands", () => {
    expect(isGithubShorthand("taskcore/companies/gstack")).toBe(true);
    expect(isGithubShorthand("taskcore/companies")).toBe(true);
  });
...
describe("normalizeGithubImportSource", () => {
  it("normalizes shorthand imports to canonical GitHub sources", () => {
    expect(normalizeGithubImportSource("taskcore/companies/gstack")).toBe(
      "https://github.com/khulnasoft/companies?ref=main&path=gstack",
    );
  });
  ...
  it("applies --ref to existing GitHub tree URLs without losing the package path", () => {
    expect(
      normalizeGithubImportSource(
        "https://github.com/khulnasoft/companies/tree/main/gstack",
        "release/2026-03-23",
      ),
    ).toBe(
      "https://github.com/khulnasoft/companies?ref=release%2F2026-03-23&path=gstack",
    );
  });
});
