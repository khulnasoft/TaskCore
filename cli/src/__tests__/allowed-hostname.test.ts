...
function writeBaseConfig(configPath: string) {
  const base: TaskcoreConfig = {
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/taskcore-db",
      embeddedPostgresPort: 54329,
      backup: {
        enabled: true,
        intervalMinutes: 60,
        retentionDays: 30,
        dir: "/tmp/taskcore-backups",
      },
    },
    logging: {
      mode: "file",
      logDir: "/tmp/taskcore-logs",
    },
    server: {
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/taskcore-storage" },
      s3: {
        bucket: "taskcore",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/taskcore-secrets/master.key" },
    },
  };
