...
function buildSourceConfig(): TaskcoreConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("taskcore-pr-432")).toBe(
      path.resolve(os.homedir(), "taskcore-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("taskcore/pr-432")).toThrow(
  ...
  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/taskcore-feature",
      homeDir: "/tmp/taskcore-worktrees",
    ...
    expect(env.TASKCORE_HOME).toBe(path.resolve("/tmp/taskcore-worktrees"));
    expect(env.TASKCORE_INSTANCE_ID).toBe("feature-worktree-support");
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    ...
    try {
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.TASKCORE_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.TASKCORE_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.TASKCORE_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          TASKCORE_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    ...
    try {
      ...
      await worktreeInitCommand({
        ...
        home: path.join(tempRoot, ".taskcore-worktrees"),
      });
      ...
      if (originalJwtSecret === undefined) {
        ...
        process.env.TASKCORE_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "TASKCORE_INSTANCE_ID=pap-1132-chat",
        ].join("\n"),
        "utf8",
      );
      expect(
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/taskcore",
        targetRepoRoot: "/Users/example/taskcore-pr-432",
        workspaceCwd: "/Users/example/taskcore",
      }),
    ).toBe("/Users/example/taskcore-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/taskcore",
        targetRepoRoot: "/Users/example/taskcore-pr-432",
        workspaceCwd: "/Users/example/taskcore/packages/db",
      }),
    ).toBe("/Users/example/taskcore-pr-432/packages/db");
  });
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    ...
    try {
      ...
      await worktreeMakeCommand("taskcore-make-test", {
        seed: false,
        home: path.join(tempRoot, ".taskcore-worktrees"),
      });
