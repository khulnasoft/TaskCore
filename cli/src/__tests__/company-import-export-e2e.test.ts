...
function createCliEnv() {
  ...
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.TASKCORE_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.TASKCORE_MIGRATION_AUTO_APPLY;
  delete env.TASKCORE_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `taskcore run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("taskcore company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
