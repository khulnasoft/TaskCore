...
function makeConfigPath(root: string, enabled: boolean): string {
  const configPath = path.join(root, ".taskcore", "config.json");
...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "taskcore-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.TASKCORE_HOME = path.join(root, "home");
    process.env.TASKCORE_INSTANCE_ID = "telemetry-test";
