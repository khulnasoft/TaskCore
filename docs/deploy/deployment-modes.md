...
Taskcore supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
When migrating from `local_trusted` to `authenticated`, Taskcore emits a one-time claim URL at startup:
...
```sh
TASKCORE_DEPLOYMENT_MODE=authenticated TASKCORE_BIND=lan pnpm taskcore run
```
