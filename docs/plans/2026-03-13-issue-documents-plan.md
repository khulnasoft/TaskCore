...
Add first-class **documents** to Taskcore as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Taskcore workflow/docs.
...
Update the Taskcore agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/taskcore/SKILL.md`
