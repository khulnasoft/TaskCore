...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Taskcore.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.taskcore.yaml`
...
- Taskcore must not require extra top-level fields for skill validity
- Taskcore-specific extensions must live under `metadata.taskcore` or `metadata.sources`
...
For Taskcore, the preferred fidelity extension is:
...
- issue/task Taskcore-only metadata
...
- Taskcore tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Taskcore shape:
...
Additional rules for Taskcore exporters:
...
- omit empty and default-valued Taskcore fields when possible
...
- Taskcore exporters should emit `.taskcore.yaml` as a sidecar by default
...
Taskcore can map this spec to its runtime model like this:
...
- Taskcore extension:
  - `.taskcore.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Taskcore-specific fidelity

Inline Taskcore-only metadata that must live inside a shared markdown file should use:

- `metadata.taskcore`

That keeps the base format broader than Taskcore.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Taskcore.

## 20. Cutover

Taskcore should cut over to this markdown-first package model as the primary portability format.

`taskcore.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Taskcore, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
