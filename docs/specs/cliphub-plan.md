# ClipHub: Marketplace for Taskcore Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Taskcore configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Taskcore-managed companies.
...
A complete Taskcore company configuration:
...
Single-agent configurations designed to plug into a Taskcore org:
...
Portable skill files that any Taskcore agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Taskcore's skill loading system
...
interface Listing {
  ...
  taskcoreVersionMin: string; // Minimum Taskcore version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null; // Target Taskcore company
...
| `POST`   | `/api/listings/:id/install`  | Install to Taskcore company                                  |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Taskcore company (or create new)
  → ClipHub API calls Taskcore API to:
  ...
  → Redirect to Taskcore dashboard with new team running
```
...
### 5.3 Creator: Export from Taskcore → Publish

```
Running Taskcore company → "Export as Blueprint" (CLI or UI)
  → Taskcore exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Taskcore brand blue/purple)
...
| Install flow | One-click deploy to Taskcore company        |
...
{
  "targetCompanyId": "uuid",         // Existing Taskcore company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `taskcore-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Taskcore
- **Backend**: Node.js API (or extend Taskcore server)
- **Database**: Postgres (can share Taskcore's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Taskcore auth (or OAuth2)

### 9.2 Integration with Taskcore

ClipHub can be:

- **Option A**: A separate app that calls Taskcore's API to install blueprints
- **Option B**: A built-in section of the Taskcore UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Taskcore UI and API.
...
- [ ] Install flow: blueprint → Taskcore company
...
- [ ] "Export from Taskcore" CLI command
