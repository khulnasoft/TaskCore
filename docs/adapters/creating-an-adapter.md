...
Build a custom adapter to connect Taskcore to any agent runtime.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@taskcore/adapter-utils/server-utils`
2. Build environment with `buildTaskcoreEnv(agent)` plus context vars
...
For built-in adapters registered in Taskcore's source:
...
`format-event.ts` — pretty-prints stdout for `taskcore run --watch` using `picocolors`.
...
Make Taskcore skills discoverable to your agent runtime without writing to the agent's working directory:
