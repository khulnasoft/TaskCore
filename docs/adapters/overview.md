...
summary: What adapters are and how they connect agents to Taskcore
---

Adapters are the bridge between Taskcore's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, Taskcore:
...
You can build and distribute adapters as standalone packages — no changes to Taskcore's source code required. External adapters are loaded at startup via the plugin system.
...
External adapters can ship a self-contained UI parser that tells the Taskcore web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.
