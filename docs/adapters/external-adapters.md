...
summary: Build, package, and distribute adapters as plugins without modifying Taskcore source
---

Taskcore supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Taskcore's source code.
...
```json
{
  ...
  "dependencies": {
    "@taskcore/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
```ts
import type { ServerAdapterModule } from "@taskcore/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with Taskcore vars injected
  const env = buildTaskcoreEnv(agent);
...
```sh
# Via the Taskcore UI
# Settings → Adapters → Install from npm → "my-taskcore-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-taskcore-adapter"}'
```
...
Local adapters are symlinked into Taskcore's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.taskcore/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-taskcore-adapter",
...
```ts
import type { AdapterSessionCodec } from "@taskcore/adapter-utils";
...
Other Taskcore users can then install your adapter by package name from the UI or API.
