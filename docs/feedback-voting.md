...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), Taskcore saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
```bash
# Installed CLI
taskcore feedback report

# Point to a different server or company
pnpm taskcore feedback report --api-base http://127.0.0.1:3000 --company-id <company-id>

# Include raw payload dumps in the report
pnpm taskcore feedback report --payloads
```
...
Exports are full by default. `traces/` keeps the Taskcore envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.

```bash
# Custom server and output directory
pnpm taskcore feedback export --api-base http://127.0.0.1:3000 --company-id <company-id> --out ./my-export
```
...
- Default endpoint: when no feedback export backend URL is configured, Taskcore falls back to `https://telemetry.taskcore.ing`
