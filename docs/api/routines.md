...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Taskcore-Signature` + `X-Taskcore-Timestamp` header pair matching the trigger's signing mode.
