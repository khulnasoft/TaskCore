...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Taskcore.

Taskcore now presents this as a workspace-command model:
...
- Taskcore does not automatically start or stop these workspace services as part of issue execution.
- Taskcore also does not automatically restart workspace services on server boot.
...
2. Taskcore realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Taskcore persists execution-workspace metadata such as paths, refs, and provisioning settings.
