...
Taskcore companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .taskcore.yaml     # Adapter config, env inputs, routines
```
...
```sh
taskcore company export <company-id> --out ./my-export
```
...
taskcore company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
taskcore company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
taskcore company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.taskcore.yaml`
...
```sh
# From a local folder
taskcore company import ./my-export

# From a GitHub URL
taskcore company import https://github.com/org/repo

# From a GitHub subfolder
taskcore company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
taskcore company import org/repo
taskcore company import org/repo/companies/acme
```
...
If `--target` is not specified, Taskcore infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
taskcore company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
taskcore company import org/company-templates/engineering-team \
  --target new \
...
```sh
taskcore company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
taskcore company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
taskcore company import ./package \
  --target new \
  --yes \
  --json
```
...
Taskcore supports several GitHub URL formats:
