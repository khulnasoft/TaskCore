...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Taskcore.
...
If `TASKCORE_APPROVAL_ID` is set, handle the approval first:
...
- If `TASKCORE_TASK_ID` is set and assigned to you, prioritize it
