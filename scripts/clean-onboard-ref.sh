...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Taskcore data dir to use
  TASKCORE_HOST=127.0.0.1    Host passed to the onboarded server
  TASKCORE_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/taskcore-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/taskcore-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export TASKCORE_HOST="${TASKCORE_HOST:-127.0.0.1}"
export TASKCORE_PORT="${TASKCORE_PORT:-3100}"
export TASKCORE_OPEN_ON_LISTEN="${TASKCORE_OPEN_ON_LISTEN:-false}"
...
echo "TASKCORE_HOST: $TASKCORE_HOST"
echo "TASKCORE_PORT: $TASKCORE_PORT"
...
env \
  ...
  HOST="$TASKCORE_HOST" \
  PORT="$TASKCORE_PORT" \
  TASKCORE_OPEN_ON_LISTEN="$TASKCORE_OPEN_ON_LISTEN" \
  pnpm taskcore onboard --yes --data-dir "$PC_DATA"
