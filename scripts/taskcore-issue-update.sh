...
usage() {
  cat <<'EOF'
Usage:
  scripts/taskcore-issue-update.sh [--issue-id ID] [--status STATUS] [--comment TEXT] [--dry-run]
...
Examples:
  ...
  scripts/taskcore-issue-update.sh --issue-id "$TASKCORE_TASK_ID" --status done --dry-run <<'MD'
  Done
...
if [[ -z "$issue_id" ]]; then
  printf 'Missing issue id. Pass --issue-id or set TASKCORE_TASK_ID.\n' >&2
  exit 1
fi
...
if [[ -z "${TASKCORE_API_URL:-}" || -z "${TASKCORE_API_KEY:-}" || -z "${TASKCORE_RUN_ID:-}" ]]; then
  printf 'Missing TASKCORE_API_URL, TASKCORE_API_KEY, or TASKCORE_RUN_ID.\n' >&2
  exit 1
fi

curl -sS -X PATCH \
  "$TASKCORE_API_URL/api/issues/$issue_id" \
