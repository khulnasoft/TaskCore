...
const DEFAULT_QUERY = "\"Co-Authored-By: Taskcore <noreply@taskcore.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/taskcore-commit-metrics-cache.json");
...
const TASKCORE_EMAIL = "noreply@taskcore.ing";
const TASKCORE_NAME = "taskcore";
...
function printHelp() {
  console.log(`Usage: tsx scripts/taskcore-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === TASKCORE_EMAIL) || (displayName && displayName.toLowerCase() === TASKCORE_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Taskcore commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "taskcore-commit-metrics",
