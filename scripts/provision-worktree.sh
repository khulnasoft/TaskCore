...
base_cwd="${TASKCORE_WORKSPACE_BASE_CWD:?TASKCORE_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${TASKCORE_WORKSPACE_CWD:?TASKCORE_WORKSPACE_CWD is required}"
taskcore_home="${TASKCORE_HOME:-$HOME/.taskcore}"
taskcore_instance_id="${TASKCORE_INSTANCE_ID:-default}"
taskcore_dir="$worktree_cwd/.taskcore"
worktree_config_path="$taskcore_dir/config.json"
worktree_env_path="$taskcore_dir/.env"
worktree_name="${TASKCORE_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${TASKCORE_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.taskcore/config.json" || -L "$base_cwd/.taskcore/config.json" ) ]]; then
  source_config_path="$base_cwd/.taskcore/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$taskcore_home/instances/$taskcore_instance_id/config.json"
fi
...
write_fallback_worktree_config() {
  ...
  TASKCORE_DIR="$taskcore_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  TASKCORE_WORKTREES_DIR="${TASKCORE_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const taskcoreDir = process.env.TASKCORE_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.TASKCORE_WORKTREES_DIR) ?? "~/.taskcore-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(taskcoreDir, "config.json");
  const envPath = path.resolve(taskcoreDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "taskcore",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.TASKCORE_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.TASKCORE_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.TASKCORE_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "TASKCORE_HOME=" + JSON.stringify(worktreeHome),
    "TASKCORE_INSTANCE_ID=" + JSON.stringify(instanceId),
    "TASKCORE_CONFIG=" + JSON.stringify(configPath),
    "TASKCORE_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "TASKCORE_IN_WORKTREE=true",
    "TASKCORE_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.TASKCORE_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("TASKCORE_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".taskcore-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
