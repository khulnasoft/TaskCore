#!/usr/bin/env node
/**
 * Screenshot utility for Taskcore UI.
 *
 * Reads the board token from ~/.taskcore/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/taskcore-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".taskcore/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.taskcore/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Taskcore origin only
