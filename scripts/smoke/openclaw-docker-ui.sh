...
TASKCORE_HOST_PORT="${TASKCORE_HOST_PORT:-3100}"
TASKCORE_HOST_FROM_CONTAINER="${TASKCORE_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.taskcore-openclaw.override.yml"
...
detect_taskcore_base_url() {
  ...
  for candidate in "$TASKCORE_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${TASKCORE_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${TASKCORE_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
taskcore_base_url="$(detect_taskcore_base_url || true)"
...
Taskcore URL for OpenClaw container:
EOF
  if [[ -n "$taskcore_base_url" ]]; then
    cat <<EOF
  $taskcore_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${TASKCORE_HOST_PORT}
  (Do not use http://127.0.0.1:${TASKCORE_HOST_PORT} inside the container.)
  If Taskcore rejects the host, run on host machine:
    pnpm taskcore allowed-hostname host.docker.internal
  Then restart Taskcore and re-run this script.
EOF
  fi
  cat <<EOF
