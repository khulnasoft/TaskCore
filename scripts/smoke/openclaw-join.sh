...
COMPANY_ID="${COMPANY_ID:-${TASKCORE_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-taskcore-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-taskcore-openclaw-smoke}"
...
Provide one of:
  TASKCORE_AUTH_HEADER=\"Bearer <board-token>\"
  TASKCORE_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Taskcore OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.taskcore.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
