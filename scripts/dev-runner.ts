...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[taskcore] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`taskcore worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const ignoredRelativePaths = new Set([
  ".taskcore/dev-server-status.json",
]);
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[taskcore] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[taskcore] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[taskcore] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  TASKCORE_UI_DEV_MIDDLEWARE: "true",
};
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[taskcore] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.TASKCORE_BIND = effectiveBind;
  if (bindHost) {
    env.TASKCORE_BIND_HOST = bindHost;
  } else {
    delete env.TASKCORE_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.TASKCORE_DEPLOYMENT_MODE;
    delete env.TASKCORE_DEPLOYMENT_EXPOSURE;
    delete env.TASKCORE_AUTH_BASE_URL_MODE;
    console.log("[taskcore] dev mode: local_trusted (bind=loopback)");
  } else {
    env.TASKCORE_DEPLOYMENT_MODE = "authenticated";
    env.TASKCORE_DEPLOYMENT_EXPOSURE = "private";
    env.TASKCORE_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[taskcore] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.TASKCORE_BIND;
  delete env.TASKCORE_BIND_HOST;
  delete env.TASKCORE_DEPLOYMENT_MODE;
  delete env.TASKCORE_DEPLOYMENT_EXPOSURE;
  delete env.TASKCORE_AUTH_BASE_URL_MODE;
  console.log("[taskcore] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[taskcore] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@taskcore/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.TASKCORE_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[taskcore] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[taskcore] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@taskcore/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[taskcore] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@taskcore/server", serverScript, ...forwardedArgs],
