...
/**
 * Filter .taskcore.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterTaskcoreYaml(yaml: string, checkedFiles: Set<string>): string {
...
export function CompanyExport() {
  ...
  // Recompute .taskcore.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .taskcore.yaml
    const yamlPath = exportData.taskcoreExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterTaskcoreYaml(exportData.files[yamlPath], checkedFiles);
    }
