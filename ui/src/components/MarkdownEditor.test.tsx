...
import { buildProjectMentionHref, buildSkillMentionHref } from "@taskcore/shared";
import {
...
describe("MarkdownEditor", () => {
  ...
  it("keeps mention queries active across spaces", () => {
    expect(findMentionMatch("Ping @Taskcore App", "Ping @Taskcore App".length)).toEqual({
      trigger: "mention",
      marker: "@",
      query: "Taskcore App",
      atPos: 5,
      endPos: "Ping @Taskcore App".length,
    });
  });
  ...
  it("accepts mention selection from touchstart taps", async () => {
    ...
    const option = Array.from(document.body.querySelectorAll('button[type="button"]'))
      .find((node) => node.textContent?.includes("Taskcore App"));
    ...
    expect(handleChange).toHaveBeenCalledWith(
      `[@Taskcore App](${buildProjectMentionHref("project-123", "#336699")}) `,
    );
