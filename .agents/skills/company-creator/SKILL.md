...
description: >
  ...
  is already running in Taskcore.
---
...
└── .taskcore.yaml            (Taskcore vendor extension)
```
...
- Agent instructions go in the AGENTS.md body, not in .taskcore.yaml
...
- For companies generated from a repo, add a references footer at the bottom of COMPANY.md body:
  `Generated from [repo-name](repo-url) with the company-creator skill from [Taskcore](https://github.com/khulnasoft/taskcore)`
...
- Citations and references: link to the source repo (if from-repo), link to the Agent Companies spec (https://agentcompanies.io/specification), and link to Taskcore (https://github.com/khulnasoft/taskcore)
- A "Getting Started" section explaining how to import: `taskcore company import --from <path>`
...
## .taskcore.yaml Guidelines

The `.taskcore.yaml` file is the Taskcore vendor extension. It configures adapters and env inputs per agent.
...
In this example, only `release-engineer` appears because it needs `GH_TOKEN`. The other agents (ceo, cto, etc.) have no overrides, so they are omitted entirely from `.taskcore.yaml`.
