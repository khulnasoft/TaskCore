...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveTaskcoreHomeDir(), "instances", resolveTaskcoreInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".taskcore", CONFIG_BASENAME);
...
function resolveTaskcoreConfigPath(): string {
  if (process.env.TASKCORE_CONFIG?.trim()) {
    return path.resolve(process.env.TASKCORE_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveTaskcoreEnvPath(configPath: string): string {
