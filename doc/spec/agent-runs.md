...
This spec defines how Taskcore actually runs agents while staying runtime-agnostic.
...
1. Taskcore is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Taskcore manages lifecycle, and on exit Taskcore parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `taskcore.skill` (shared Taskcore skill text block)
...
2. Preferred transport is env vars (`TASKCORE_*`) injected at runtime.
