...
This document defines the V1 design for a Taskcore adapter that integrates with
...
4. synthesized stdout events for Taskcore UI/CLI
...
3. **Agent callback auth to Taskcore** uses a bootstrap exchange flow (no long-lived Taskcore key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Taskcore endpoints, not full SKILL.md prompt injection.
...
- `taskcorePublicUrl` (optional override; else `TASKCORE_PUBLIC_URL` env)
...
1. `adapterConfig.taskcorePublicUrl`
2. `process.env.TASKCORE_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Taskcore credentials in prompt text.

Flow:

1. Before launch/follow-up, Taskcore mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Taskcore validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Taskcore API calls.
...
1. Prompt includes a compact instruction to fetch skills from Taskcore.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/taskcore`
   - `GET /api/skills/taskcore-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <taskcorePublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Taskcore run context.
...
5. `taskcorePublicUrl` present and reachable shape-valid
...
| Skill delivery  | local skill dir injection      | authenticated fetch from Taskcore skill endpoints |
| Taskcore auth   | injected local run JWT env var | bootstrap token exchange -> run JWT               |
...
2. Attach image payloads from Taskcore context.
3. Add richer PR metadata surfacing in Taskcore UI.
