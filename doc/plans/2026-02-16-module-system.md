...
Taskcore's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
| -------------------- | --------------------------------------------------------------------------------- | -------------- |
| **Module**           | A package that extends Taskcore's API, UI, and data model                         | Yes            |
...
modules/
  observability/
    taskcore.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@taskcore/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@taskcore/core";
...
```
1. Scan modules/ directory for taskcore.module.json manifests
...
```jsonc
// taskcore.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@taskcore/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    placement: "dashboard",
    component: lazy(() =>
      import("@taskcore/mod-observability/ui").then((m) => ({
        default: m.TokenBurnRateWidget,
      })),
    ),
  },
];
  ...
  "modules": [
    {
      ...
      "repo": "github:taskcore/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.taskcore.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm taskcore store list                    # browse available modules and templates
pnpm taskcore store install <module-id>     # install a module
pnpm taskcore store import <template-id>    # import a company template
pnpm taskcore store export                  # export current company as template
```
...
| **Multi-tenant**        | Separate companies/orgs within one Taskcore instance           | `server:started`  |
...
4. **Module config** — `taskcore.config.json` with per-module config, env var interpolation
...
10. **`@taskcore/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
