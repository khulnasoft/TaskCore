...
This document defines the product and UI plan for skill management in Taskcore.
...
- Taskcore company import/export can include skills as package content
...
3. The UI must always tell the truth about what Taskcore knows:
   - desired state in Taskcore
...
Taskcore should treat skills at two scopes:
...
- created locally inside Taskcore later
...
7. “Understand what Taskcore knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Taskcore-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Taskcore did not install.

These should be clearly marked:

- external
- not managed by Taskcore
...
- `desired_only`: Taskcore wants it, adapter does not show it yet
- `external`: adapter has it but Taskcore does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Taskcore only tracks desired state
...
- Taskcore can still track desired skills
...
- Taskcore can see adapter skills
...
- `.taskcore.yaml` may add Taskcore-specific fidelity, but should not replace the base shortname association model
...
- `.taskcore.yaml` may add Taskcore-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Taskcore should have a company-scoped skill model or managed package model representing:
...
Taskcore should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Taskcore-specific.
...
That gives Taskcore one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
