...
`PAP-447` asks how Taskcore should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Taskcore instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Taskcore:

- some operators use Taskcore to manage Taskcore and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Taskcore
- Claude and Codex expose different built-in affordances, so Taskcore should not overfit to one tool

## Core Product Decision

Taskcore should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Taskcore
...
- Taskcore should not assume that every adapter can see or use a host filesystem path directly
...
If Taskcore makes worktrees agent-first, it will blur:
...
- Taskcore core resolves and records execution workspace state
...
That gives Taskcore room to support:
...
- Taskcore handles workspace runtime services under the hood using project/workspace policy
...
Once Taskcore is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Taskcore should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Taskcore cannot infer
...
- when issue moves to done, should Taskcore auto-commit?
- should it auto-push?
...
Taskcore already has the right foundation for a project-first model.
...
For server-spawned local adapters, Taskcore already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Taskcore should mirror directly

Implication:

- for `codex_local`, Taskcore should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Taskcore should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Taskcore should separate:
...
Taskcore should be able to express intentions such as:
...
These are not specific to Taskcore. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".taskcore/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Taskcore-specific `devServer` flag.
...
Taskcore should distinguish between:
...
Taskcore should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Taskcore should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Taskcore
...
For runtime services specifically, Taskcore should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Taskcore-only dev-server flags
...
3. Inject resolved execution workspace details into `context.taskcoreWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Taskcore instance can be reused through env/config injection when appropriate
...
- Taskcore can decide whether to reuse or start a fresh service deterministically
...
3. Test against Taskcore-on-Taskcore workflows first.
...
7. Workspace runtime services are modeled generically, not as Taskcore-specific dev-server toggles.
