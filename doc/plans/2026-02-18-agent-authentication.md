...
Agents need API keys to authenticate with Taskcore. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Taskcore
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Taskcore generates a token and passes it directly to the agent
...
`TASKCORE_API_URL`, `TASKCORE_AGENT_ID`, `TASKCORE_COMPANY_ID`. We need to
add a `TASKCORE_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Taskcore CLI
...
```
taskcore auth login
# Opens browser -> user confirms -> token stored at ~/.taskcore/credentials
```
...
managed by the Taskcore server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Taskcore.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Taskcore SKILL.md (or a link to it)
   - What information Taskcore needs from the agent (e.g. webhook URL, adapter
...
5. Taskcore stores the pending registration.
...
7. On approval, Taskcore provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Taskcore issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.taskcore.ing/skills/taskcore/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Taskcore should send you heartbeats",
...
1. Generate a Taskcore invite link for the company.
...
4. A Taskcore company member approves the new agent.
5. Taskcore begins sending heartbeats to the OpenClaw webhook endpoint.
