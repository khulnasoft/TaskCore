...
This document defines the next-stage plan for Taskcore company import/export.

The core shift is:

- move from a Taskcore-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Taskcore.
...
Taskcore already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Taskcore
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `taskcore.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.taskcore.yaml` sidecar for high-fidelity Taskcore-specific details
...
11. adapter-aware skill sync surfaces so Taskcore can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Taskcore at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Taskcore sidecar.
...
Taskcore must not redefine `SKILL.md`.
...
- Taskcore-specific fidelity lives in `.taskcore.yaml`
- Taskcore may resolve and install `SKILL.md` packages, but it must not require a Taskcore-only skill format
...
### 5.4 Base Package Vs Taskcore Extension
...
- base package:
  ...
  - no Taskcore-only runtime fields by default
- Taskcore extension:
  - `.taskcore.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Taskcore tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`taskcore.manifest.json` is not part of the future package direction.
...
Taskcore import/export should support these entity kinds:
...
- Taskcore can treat “this set of agents came from team package X” as the imported-team model
...
Taskcore should support source references in package metadata with:
...
Taskcore should define a new adapter surface area around skills:
...
Planned Taskcore behavior:

- if an adapter supports read, Taskcore should show current skills in the UI
- if an adapter supports write, Taskcore should let the user enable/disable imported skills
- if an adapter supports sync, Taskcore should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.taskcore.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.taskcore.yaml` alongside the base package
...
## 10. Storage Model Inside Taskcore
...
Taskcore should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `taskcore company export <company-id> --out <path>`
- `taskcore company import <path-or-url> --dry-run`
- `taskcore company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Taskcore storage, or only referenced at import time?
...
3. Should Taskcore support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Taskcore aligned with:
