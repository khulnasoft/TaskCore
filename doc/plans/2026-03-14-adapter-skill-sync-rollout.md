...
This document defines the rollout plan for adapter-wide skill support in Taskcore.
...
- Taskcore stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Taskcore currently has these adapters:
...
1. Desired skills live in Taskcore for every adapter.
...
These adapters have a stable local skills directory that Taskcore can read and manage.
...
These adapters do not have a meaningful Taskcore-owned persistent install state.
...
- desired skills stored in Taskcore
...
- runtime already injects Taskcore skills into `~/.cursor/skills`
...
- runtime already injects Taskcore skills into `~/.gemini/skills`
...
- runtime already injects Taskcore skills into `~/.pi/agent/skills`
...
- Taskcore can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Taskcore skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Taskcore may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Taskcore skills that are clearly marked as Taskcore-managed
...
- Taskcore stores desired skills only
...
- avoid destructive wording unless Taskcore can prove a skill is Taskcore-managed
...
That gets Taskcore from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
