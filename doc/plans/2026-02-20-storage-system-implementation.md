...
Add a single storage subsystem for Taskcore that supports:
...
- API serves attachment bytes through authenticated Taskcore endpoints.
...
- Default local path is under instance root: `~/.taskcore/instances/<instanceId>/data/storage`.
...
- `taskcore onboard` writes a valid `storage` config block by default.
- `taskcore configure --section storage` can switch between local and s3 modes.
...
- `taskcore doctor` reports actionable storage status.
