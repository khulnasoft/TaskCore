...
1. Start Taskcore in auth mode.

```bash
cd <taskcore-repo-root>
pnpm dev --bind lan
```
...
3. In Taskcore UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in Taskcore UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from Taskcore invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in Taskcore with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in Taskcore UI that new issue exists.

10. Watch logs during test (optional but helpful):

```bash
docker compose -f /tmp/openclaw-docker/docker-compose.yml -f /tmp/openclaw-docker/.taskcore-openclaw.override.yml logs -f openclaw-gateway
```
