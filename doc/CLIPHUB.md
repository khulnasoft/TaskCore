...
ClipHub is the public registry where people share, discover, and download Taskcore company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Taskcore what a package registry is to a programming language. Taskcore already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Taskcore — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Taskcore instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Taskcore spec. It contains:
...
```
taskcore install cliphub:<publisher>/<company-slug>
```

Downloads the template and creates a new company in your local Taskcore instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Taskcore, export your company as a template, then publish:

```
taskcore export --template my-company
taskcore publish cliphub my-company
```
...
ClipHub is a **separate service** from Taskcore itself. Taskcore is self-hosted; ClipHub is a hosted registry that Taskcore instances talk to.
...
| **Taskcore CLI** | `taskcore install`, `taskcore publish`, `taskcore cliphub sync` — built into Taskcore           |
| **Taskcore UI**  | "Browse ClipHub" panel in the Taskcore web UI for discovering templates without leaving the app |

### Tech Stack

| Layer    | Technology                                                |
| -------- | --------------------------------------------------------- |
| Frontend | React + Vite (consistent with Taskcore)                   |
| Backend  | TypeScript + Hono (consistent with Taskcore)              |
| Database | PostgreSQL                                                |
...
4. Run `taskcore install cliphub:acme/lean-saas-shop`
5. Taskcore creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Taskcore until it works well
2. Export: `taskcore export --template my-agency`
3. Publish: `taskcore publish cliphub my-agency`
...
3. Install just that agent: `taskcore install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Taskcore

ClipHub is **not required** to use Taskcore. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Taskcore what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `taskcore install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Taskcore web UI
- [ ] `taskcore cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Taskcore's own philosophy)
