...
Taskcore V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Taskcore-shaped design.

Assumption for this document: Taskcore is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Taskcore objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Taskcore.

The main conclusion is:

- Taskcore should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Taskcore should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Taskcore should use multiple extension classes instead of one generic plugin bag:
...
If Taskcore does this well, the examples you listed become straightforward:
...
Relevant Taskcore files reviewed for current extension seams:
...
It is too dangerous for Taskcore core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Taskcore — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Taskcore should absolutely do this.
...
Taskcore should copy this discipline.
...
Taskcore should adopt this style for plugin actions, automations, and UI schemas.
...
Taskcore can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Taskcore too.

## What Taskcore Should Not Copy Directly
...
Taskcore is a control plane for an operator-managed instance with company objects.
...
Taskcore is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Taskcore should not auto-load arbitrary code from a workspace repo like `.taskcore/plugins` or project directories.
...
For Taskcore, equivalent hooks into:
...
Taskcore should not allow plugins to silently replace:
...
For Taskcore it would be risky because it combines:
...
Taskcore should require an explicit operator install step.

## Why Taskcore Needs A Different Shape

The products are solving different problems.

| Topic                | OpenCode                          | Taskcore                                             |
| -------------------- | --------------------------------- | ---------------------------------------------------- |
| Primary unit         | local project/worktree            | single-tenant operator instance with company objects |
| Trust assumption     | local power user on own machine   | operator managing one trusted Taskcore instance      |
...
This is good news.
Taskcore does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Taskcore Plugin Model
...
Taskcore already has this pattern implicitly:
...
- `plugin` means integration code that talks to Taskcore through a typed plugin protocol and capability model
...
- invoke explicit Taskcore actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@taskcore/plugin-sdk/ui`) with design tokens and common components.
...
Taskcore should treat plugin installation as a global instance-level action.

Examples:

- install `@taskcore/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Taskcore objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Taskcore already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Taskcore too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@taskcore/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@taskcore/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Taskcore instance.

A `create-taskcore-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Taskcore achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Taskcore server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Taskcore's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@taskcore/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@taskcore/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Taskcore

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@taskcore/plugin-sdk";

export default definePlugin({
  id: "@taskcore/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Taskcore entities
    });
    ...
    ctx.events.on("plugin.@taskcore/plugin-git.push-detected", async (event) => {
...
Any Taskcore plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Taskcore objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@taskcore/plugin-linear`)
...
- reuse core tables when the data is clearly part of Taskcore itself
...
If a concept is becoming part of Taskcore's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Taskcore itself
...
### 3. Scope plugin data to Taskcore objects before adding custom schemas

A lot of plugin data naturally hangs off existing Taskcore objects:
...
If Taskcore eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@taskcore/plugin-workspace-files`
...
Package idea: `@taskcore/plugin-terminal`
...
Package idea: `@taskcore/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Taskcore.
- Board reviews the diff after a run without leaving Taskcore.
...
- optional `events.emit` (e.g. `plugin.@taskcore/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@taskcore/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@taskcore/plugin-linear`

This plugin syncs Taskcore work with Linear. It is useful for:

- importing backlog from Linear
- linking Taskcore issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Taskcore and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Taskcore.
- Taskcore issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@taskcore/plugin-github-issues`

This plugin syncs Taskcore issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Taskcore issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Taskcore task governance
...
- Plugin settings:
  ...
  - whether new Taskcore issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Taskcore issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Taskcore.
- GitHub webhooks update status/comment state in Taskcore.
- A PR is linked back to the Taskcore issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@taskcore/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@taskcore/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Taskcore. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Taskcore issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Taskcore plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Taskcore dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Taskcore issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Taskcore core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@taskcore/plugin-runtime-processes`
...
Package idea: `@taskcore/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Taskcore. It is useful for:
...
- Failed payment spikes or churn events can generate Taskcore issues for follow-up.
...
- Stripe data should stay additive to Taskcore core
...
- plugin UI bundle loading, host bridge, `@taskcore/plugin-sdk/ui`
...
- `@taskcore/plugin-test-harness` and `create-taskcore-plugin` starter template
...
Taskcore should not implement "an OpenCode-style generic in-process hook system."
Taskcore should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Taskcore
...
8. Ship `@taskcore/plugin-test-harness` and `create-taskcore-plugin` starter template.
