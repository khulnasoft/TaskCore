...
This document is the complete specification for Taskcore's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Taskcore app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Taskcore HTTP APIs directly.
...
Taskcore plugin design is based on the following assumptions:

1. Taskcore is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Taskcore business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Taskcore core.
...
2. Let plugins add major capabilities without editing Taskcore core.
...
4. Depend on project-local plugin folders such as `.taskcore/plugins`.
...
The single Taskcore deployment an operator installs and controls.

### 5.2 Company

A first-class Taskcore business object inside the instance.
...
A trusted in-process extension loaded directly by Taskcore core.
...
An installable instance-wide extension package loaded through the Taskcore plugin runtime.
...
Taskcore has two extension classes.
...
Taskcore already has a concrete workspace model:
...
Plugins live under the Taskcore instance directory.

Suggested layout:

- `~/.taskcore/instances/default/plugins/package.json`
- `~/.taskcore/instances/default/plugins/node_modules/`
- `~/.taskcore/instances/default/plugins/.cache/`
- `~/.taskcore/instances/default/data/plugins/<plugin-id>/`
...
Taskcore should add CLI commands:

- `pnpm taskcore plugin list`
- `pnpm taskcore plugin install <package[@version]>`
- `pnpm taskcore plugin uninstall <plugin-id>`
- `pnpm taskcore plugin upgrade <plugin-id> [version]`
- `pnpm taskcore plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@taskcore/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface TaskcorePluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Taskcore agents can use during runs.
...
- Taskcore server starts one worker process per installed plugin
...
Receives one typed Taskcore domain event.
...
export function definePlugin(definition: PluginDefinition): TaskcorePlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: TaskcorePluginManifestV1;
  config: {
...
- The host provides **design tokens and shared components** via `@taskcore/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@taskcore/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@taskcore/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@taskcore/plugin-sdk/ui`
...
The `@taskcore/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Taskcore secret provider system rather than a plain text input
...
1. Core Taskcore data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Taskcore objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Taskcore product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Taskcore later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Taskcore secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm taskcore plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Taskcore server. This is a normative requirement, not optional.
...
### 27.1 `@taskcore/plugin-test-harness`
...
```ts
import { createTestHarness } from "@taskcore/plugin-test-harness";
...
For developing a plugin against a running Taskcore instance:

- The operator installs the plugin from a local path: `pnpm taskcore plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-taskcore-plugin`) that scaffolds:

- `package.json` with correct `taskcorePlugin` keys
...
- `@taskcore/plugin-workspace-files`
- `@taskcore/plugin-terminal`
- `@taskcore/plugin-git`
- `@taskcore/plugin-linear`
- `@taskcore/plugin-github-issues`
- `@taskcore/plugin-grafana`
- `@taskcore/plugin-runtime-processes`
- `@taskcore/plugin-stripe`
...
- `@taskcore/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@taskcore/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@taskcore/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@taskcore/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@taskcore/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@taskcore/plugin-sdk/ui`
...
- `@taskcore/plugin-test-harness`
- `create-taskcore-plugin` starter template
...
Taskcore should not implement a generic in-process hook bag modeled directly after local coding tools.

Taskcore should implement:
...
That is the complete target design for the Taskcore plugin system.
