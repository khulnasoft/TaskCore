...
Taskcore supports two runtime modes:
...
Taskcore now treats **bind** as a separate concern from auth:
...
pnpm taskcore onboard --yes --bind lan
pnpm taskcore run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, Taskcore emits a startup warning with a one-time high-entropy claim URL.
